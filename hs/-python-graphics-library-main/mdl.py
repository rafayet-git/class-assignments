...
def p_command_mesh(p):
    ...
    if isinstance(p[2], str): # symbol/constant
        if p[2] == ':' and isinstance(p[3], str):
            cmd['args'].append(p[3]+ p[4])
        else:
            cmd['constants'] = p[2]
            cmd['args'].append(p[4] + p[5])
    if len(p) == 6 and p[2] == ':' and isinstance(p[5], str):
        cmd['cs'] = p[5]
    if len(p) == 7 and p[3] == ':' and isinstance(p[6], str):
        cmd['cs'] = p[6]
    commands.append(cmd)
